<template>
    <div class="container mx-auto p-4">
        <div class="mb-6">
            <h1 class="text-2xl font-bold mb-2">Friends</h1>
            <p class="text-gray-600">Manage your dart friends and see their statistics</p>
        </div>

        <!-- Friends List -->
        <div v-if="friends.length > 0" class="space-y-4">
            <div v-for="friend in friends" :key="friend.id" 
                class="bg-white p-4 rounded-lg shadow-sm border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <h3 class="font-medium">{{ friend.friend.name || friend.friend.email }}</h3>
                        <p class="text-sm text-gray-600">Added {{ new Date(friend.created_at).toLocaleDateString() }}</p>
                    </div>
                    <div class="flex items-center gap-2">
                        <NuxtLink 
                            :to="`/assistant/history?friend=${friend.friend.id}`"
                            class="px-4 py-2 text-blue-600 hover:bg-blue-50 rounded-lg"
                        >
                            View Games
                        </NuxtLink>
                    </div>
                </div>
            </div>
        </div>

        <!-- Empty State -->
        <div v-else class="text-center py-12">
            <div class="mb-4">
                <svg class="w-16 h-16 mx-auto text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
                        d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z" />
                </svg>
            </div>
            <h3 class="text-lg font-medium text-gray-900">No friends added yet</h3>
            <p class="mt-1 text-gray-500">Start adding friends to track your games together!</p>
        </div>
    </div>
</template>

<script setup lang="ts">
import { ref, onMounted } from 'vue';
import { useAuth } from '~/composables/useAuth.client';
import { useFriends } from '~/composables/useFriends';

interface Friend {
    id: string;
    name?: string;
    email: string;
}

interface Friendship {
    id: string;
    user_id: string;
    friend: Friend;
    created_at: string;
}

const auth = useAuth();
const { getFriendsAsync } = useFriends();
const friends = ref<Friendship[]>([]);

onMounted(async () => {
    const { user } = await auth.getUserAsync() || {};
    if (user) {
        const { data, error } = await getFriendsAsync(user.id);
        if (data && !error) {
            friends.value = data;
        }
    }
});
</script>