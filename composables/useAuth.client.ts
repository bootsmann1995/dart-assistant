...
export const useAuth = () => {
	...
	const getUserAsync = async () => {
		if (user.value && user.value.metadata != null) {
			return user.value;
		}
	...
	const getUserMetadataAsync = async (email: string) => {
		const { data, error } = await getClient().from("user_meta_data").select("*").eq("email", email).single();
	...
	const updateUserMetadataAsync = async (metadata: UserMetadata) => {
		...
		if (existing) {
			return await getClient().from("user_meta_data").update(metadata).eq("email", metadata.email);
		} else {
			return await getClient().from("user_meta_data").insert(metadata);
		}
	};
