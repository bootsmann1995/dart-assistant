...
export const useGamesStatusX01 = () => {
	...
	const getGamesBasedOnUserAsync = async (userId: string) => {
		...
		const ownGames = await getClient().from("games-x01").select("*").eq("user_id", userId);

		// Fetch games where user is in invited_users array
		const participatedGames = await getClient().from("games-x01").select("*").contains("invited_users", [userId]);

		// Combine and sort all games
		const allGames = [...(ownGames.data || []), ...(participatedGames.data || [])];
		return {
			data: allGames.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime()),
	...
	const saveGameAsync = async (userId: string, gameState: GameState) => {
		if (await isAuthenticatedAsync()) {
			...
			const invitedUsers = gameState.players.map((p) => p.user_id).filter((id) => id && id !== userId);

			return await getClient()
				.from("games-x01")
				.insert({
					user_id: userId,
					game_data: JSON.stringify(gameState), // Store the entire game state as JSON in game_data
	...
	const updateGameInviteStatusAsync = async (requestId: string, status: "accepted" | "declined" | "expired") => {
		if (await isAuthenticatedAsync()) {
			return await getClient().from("requests").update({ status }).eq("id", requestId);
		}
		throw new Error("User not authenticated");
	};

	const getInviteStatusAsync = async (requestId: string) => {
		const { data } = await getClient().from("requests").select("*").eq("id", requestId).single();
